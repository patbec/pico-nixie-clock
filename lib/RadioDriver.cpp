...
****************************************************************/
uint8_t RadioDriver::SpiTransfer(uint8_t value)
{
    SPDR = value;
    while (!(SPSR & (1 << SPIF)))
        ;
...
*OUTPUT       :none
****************************************************************/
void RadioDriver::Init(uint8_t f)
{
    SpiInit(); //spi initialization
    GDO_Set(); //GDO set
    digitalWrite(SS_PIN, HIGH);
...
*OUTPUT       :none
****************************************************************/
void RadioDriver::SpiWriteBurstReg(uint8_t addr, uint8_t *buffer, uint8_t num)
{
    uint8_t i, temp;
...
****************************************************************/
uint8_t RadioDriver::SpiReadReg(uint8_t addr)
{
    uint8_t temp, value;
...
*OUTPUT       :none
****************************************************************/
void RadioDriver::SpiReadBurstReg(uint8_t addr, uint8_t *buffer, uint8_t num)
{
    uint8_t i, temp;
...
*OUTPUT       :status value
****************************************************************/
uint8_t RadioDriver::SpiReadStatus(uint8_t addr)
{
    uint8_t value, temp;
...
*OUTPUT       :none
****************************************************************/
void RadioDriver::RegConfigSettings(uint8_t f)
{
    ...
    SpiWriteReg(CC1101_PKTCTRL1, 0x04); //two status uint8_ts will be appended to the payload of the packet,including RSSI LQI and CRC OK
                                        //No address check
    SpiWriteReg(CC1101_PKTCTRL0, 0x05); //whitening off;CRC Enable��variable length packets, packet length configured by the first uint8_t after sync word
...
*OUTPUT       :none
****************************************************************/
void RadioDriver::SendData(uint8_t *txBuffer, uint8_t size)
{
...
*OUTPUT       :none
****************************************************************/
void RadioDriver::SetReceive(void)
{
...
****************************************************************/
uint8_t RadioDriver::ReceiveData(uint8_t *rxBuffer)
{
    uint8_t size;
    uint8_t status[2];
